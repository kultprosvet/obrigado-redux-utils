...
import {transformToImmutable} from "../src/persists/transformToImmutable";
...
describe('redux helper', () => {
    ...
    it('should deep update state', () => {
        ...
        ReduxHelper.updateIn(['week', 'days', 0, 'times'], (arr) => arr.push({ time: '12:00:00' }))
        let state = store.getState() as any
        expect(state.week.toJS().days[0].times).toEqual([{ time: '12:00:00' }, { time: '12:00:00' }])

        // add new day
        ReduxHelper.updateIn(['week', 'days'], (arr) => arr.push(
            transformToImmutable({ times: [{ time: '12:00:00' }] })))
        state = store.getState() as any
        expect(state.week.toJS().days).toEqual([
            { times: [{ time: '12:00:00' }, { time: '12:00:00' }] },
            { times: [{ time: '12:00:00' }] },
        ])
        // add time to new day
        ReduxHelper.updateIn(['week', 'days', 1, 'times'], (arr) =>{
            return arr.push({ time: '12:01:00' });
        } )
        state = store.getState() as any
        expect(state.week.toJS().days).toEqual([
            { times: [{ time: '12:00:00' }, { time: '12:00:00' }] },
            { times: [{ time: '12:00:00' }, { time: '12:01:00' }] },
        ])
    })
